...
dswhole = dsera                                           # CAMSRA vs. ERA5
folding = erafoldd                                        # CAMSRA vs. ERA5, and shallow/medium/deep
camfolding = camfoldd
y_spread = np.arange(-.75*5, .75*5.1, .25)                # CAMSRA vs. ERA5 (step by .75 for CAM, .25 for ERA5)
labsname = path_ERA5 + 'ERA5_composites_deep_tpcenter_nonan_onlyERA5.nc'        # CAMSRA vs. ERA5, and shallow/medium/deep, and nan/nonan
meansname = path_ERA5 + 'ERA5_composites_deep_tpcenter_nonan_onlyERA5_mean.nc'  # CAMSRA vs. ERA5, and shallow/medium/deep, and nan/nonan
...
for tt in [0]: # [101] test timestep (no CAM deep)
    ...
    # foldmask = folding.isel(time=tt).where(camfolding.isel(time=tt).interp(lat=folding.lat,lon=folding.lon,method='nearest')<1)>0
    foldmask = folding.isel(time=tt).where(camfolding.isel(time=tt)<1)>0    
    # testidxs = argrelmin(dswhole.dp.isel(time = tt).where(foldmask,10000).values, axis=0) 
    testidxs = argrelmin(dswhole.dp.isel(time = tt).where(foldmask).fillna(10000).values, axis=0)     
    # testlats = dswhole.lat[testidxs[0]]
    # testlons = dswhole.lon[testidxs[1]]
    testlats = camfolding.lat[testidxs[0]]
    testlons = camfolding.lon[testidxs[1]]    
    ...
    labels = dswhole.label.isel(time = tt)#.where(foldmask)

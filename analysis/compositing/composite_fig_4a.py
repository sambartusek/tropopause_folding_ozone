...
labsname = path_ERA5 + 'ERA5_composites_deep_tpcenter_nonan_onlyboth.nc'        # CAMSRA vs. ERA5, and shallow/medium/deep, and nan/nonan
meansname = path_ERA5 + 'ERA5_composites_deep_tpcenter_nonan_onlyboth_mean.nc'  # CAMSRA vs. ERA5, and shallow/medium/deep, and nan/nonan
...
for tt in [0]: # [101] test timestep (no CAM deep)
    ...
    # foldmask = folding.isel(time=tt).where(camfolding.isel(time=tt).interp(lat=folding.lat,lon=folding.lon,method='nearest'))>0
    foldmask = folding.isel(time=tt).where(camfolding.isel(time=tt))>0    
